...
namespace ChessBrowser
{
  partial class Form1
  {
    ...
    private void UploadGamesToDatabase(string PGNfilename)
    {
      ...
      PGNReader dataReader = new PGNReader(PGNfilename);
      List<ChessGame> games = dataReader.GetGames();
      ...
      using (MySqlConnection conn = new MySqlConnection(connection))
      {
        try
        {
          ...
          MySqlCommand getcmd  = conn.CreateCommand();
          
          foreach (ChessGame game in games)
          {
            command.CommandText = "insert ignore into Events (eName, Site, Date) " +
            ...
            command.CommandText = "insert ignore into Players (pName, Elo) " +
                                  "values (@player, @elo) " +
                                  "on duplicate key update Elo = " +
                                  "if(@elo > Elo, @elo, Elo);";

            command.Parameters.AddWithValue("@player", game.getWhitePlayer());
            command.Parameters.AddWithValue("@elo", game.getWhiteElo());

            command.ExecuteNonQuery();
            command.Parameters.Clear();


            command.Parameters.AddWithValue("@player", game.getBlackPlayer());
            command.Parameters.AddWithValue("@elo", game.getBlackElo());
            ...
            getcmd.CommandText = "select pID from Players where pName = @wPlayer;";
            getcmd.Parameters.AddWithValue("@wPlayer", game.getWhitePlayer());

            using (MySqlDataReader reader = getcmd.ExecuteReader())
            {
                reader.Read();
                wID = int.Parse(reader["pID"].ToString());
            }

            getcmd.CommandText = "select pID from Players where pName = @bPlayer;";
            getcmd.Parameters.AddWithValue("@bPlayer", game.getBlackPlayer());

            using (MySqlDataReader reader = getcmd.ExecuteReader())
            {
                reader.Read();
                bID = int.Parse(reader["pID"].ToString());
            }

            getcmd.CommandText = "select eID from Events where " +
                                 "eName = @name and Site = @site and Date = @date;";
            getcmd.Parameters.AddWithValue("@name", game.getName());
            getcmd.Parameters.AddWithValue("@site", game.getSite());
            getcmd.Parameters.AddWithValue("@date", game.getDate());
            ...
            command.CommandText = "insert ignore into Games values " +
                                  "(@round, @result, @moves, @bPlayer, @wPlayer, @eID);";

            command.Parameters.AddWithValue("@round", game.getRound());
            command.Parameters.AddWithValue("@result", game.getResult());
            command.Parameters.AddWithValue("@moves", game.getMoves());
            command.Parameters.AddWithValue("@bPlayer", bID);
            command.Parameters.AddWithValue("@wPlayer", wID);
            command.Parameters.AddWithValue("@eID", eID);

            command.ExecuteNonQuery();
          }
          
          // TODO: iterate through your data and generate appropriate insert commands -- <COMPLETED>
      ...
      string winner, bool useDate, DateTime start, DateTime end, bool showMoves)
    {
      ...
      using (MySqlConnection conn = new MySqlConnection(connection))
      {
        try
        {
          ...
          // For when there is one, or no entries for names
          if ((white == "" && black == "") || (white != "" && black == "") || (white == "" && black != ""))
          { 
              // BASELINE QUERY SETUP
              command.CommandText = "Select eName, Site, Date, Result, pName, Elo, pID";
              if (showMoves == true)
                command.CommandText += ", Moves";
              command.CommandText += " from Players join Games natural join Events where";
          
              // WHITE ENTRY, NO BLACK ENTRY
              if (white != "" && black == "")
              {
                if (moreParameters == true)
                  command.CommandText += " and";
                moreParameters = true;
                command.CommandText += (" pName = @white and pID = WhitePlayer");
                command.Parameters.AddWithValue("@white", white);
              }
              // NO WHITE ENTRY, BLACK ENTRY
              else if (white == "" && black != "")
              {
                if (moreParameters == true)
                  command.CommandText += " and";
                moreParameters = true;
                command.CommandText += (" pName = @black and pID = BlackPlayer");
              ...
              if (white == "" && black == "")
              {
                if (moreParameters == true)
                  command.CommandText += " and";
                moreParameters = true;
                command.CommandText += (" pID = WhitePlayer");
                command.Parameters.AddWithValue("@white", white);
              }
              ...
              if (opening != "")
              {
                if (moreParameters == true)
                  command.CommandText += " and";
                moreParameters = true;
                command.CommandText += (" Moves like @opening");
              ...
              if (winner != "")
              {
                if (moreParameters == true)
                  command.CommandText += " and";
                moreParameters = true;
                command.CommandText += (" Result = @winner");
              ...
              if (useDate == true)
              {
                if (moreParameters == true)
                  command.CommandText += " and";
                moreParameters = true;
                command.CommandText += (" Date >= @start and Date <= @end");
              ...
              List<int> otherPlayerElo = new List<int>();

              using (MySqlDataReader reader = command.ExecuteReader())
              {
                while (reader.Read())
                {
                  playerIDs.Add(int.Parse(reader["pID"].ToString()));
                }
              }
              // WHITE ENTRY, NO BLACK ENTRY
              if (white != "" && black == "")
              {
                ...
                command.CommandText = command.CommandText.Replace("pName = @white and pID = WhitePlayer", "BlackPlayer = pID and WhitePlayer = @id");
                Console.WriteLine(command.CommandText);
                command.Parameters.AddWithValue("@id", "placeholder");
                command.Prepare();
                ...
                { 
                  ...
                  using (MySqlDataReader reader = command.ExecuteReader())
                  {
                    while (reader.Read())
                    {
                      otherPlayerNames.Add(reader["pName"].ToString());
                      otherPlayerElo.Add(int.Parse(reader["Elo"].ToString()));
                    }
                  }
                }
              ...
              {
                string original = command.CommandText;
                command.CommandText = command.CommandText.Replace("pName = @black and pID = BlackPlayer", "WhitePlayer = pID and BlackPlayer = @id");
                Console.WriteLine(command.CommandText);
                command.Parameters.AddWithValue("@id", "placeholder");
                command.Prepare();
                foreach (int id in playerIDs)
              ...
              {
                string original = command.CommandText;
                command.CommandText = command.CommandText.Replace("pID = WhitePlayer", "BlackPlayer = pID and WhitePlayer = @id");
                Console.WriteLine(command.CommandText);
                command.Parameters.AddWithValue("@id", "placeholder");
                command.Prepare();
                foreach (int id in playerIDs)
              ...
              //command.CommandText = "Select eName, Site, Date, Result from Players join Games natural join Events where pName = \"Carlsen, Magnus\" and Result = @trial and WhitePlayer = pID;";
              //command.Parameters.AddWithValue("@trial", winner[0]);
              ...
              using (MySqlDataReader reader = command.ExecuteReader())
              {
                while (reader.Read())
                {
                  ...
                  // WHITE ENTRY, NO BLACK ENTRY -- OR -- NO ENTRIES
                  if ((white != "" && black == "" ) || (white == "" && black == ""))
                  { 
                    parsedResult += "White: " + reader["pName"] + " (" + reader["Elo"] + ") \r\n";
                    parsedResult += "Black: " + otherPlayerNames[0] + " (" + otherPlayerElo[0] + ") \r\n";  
                  }
                  // NO WHITE ENTRY, BLACK ENTRY
                  else if (white == "" && black != "")
                  { 
                    parsedResult += "White: " + otherPlayerNames[0] + " (" + otherPlayerElo[0] + ") \r\n"; 
                    parsedResult += "Black: " + reader["pName"] + " (" + reader["Elo"] + ") \r\n";  
                  }
                  otherPlayerElo.RemoveAt(0);
                  otherPlayerNames.RemoveAt(0);
          ...
          // For when there are two name entries
          else
          {
            int blackPlayer;
            int whitePlayer;
            ...
            command.CommandText = "select Elo, pID from Players where pName = @name;";

            command.Parameters.AddWithValue("@name", white);
            using (MySqlDataReader reader = command.ExecuteReader())
            {
              reader.Read();
              whiteElo = int.Parse(reader["Elo"].ToString());
              whitePlayer = int.Parse(reader["pID"].ToString());
            }
            command.Parameters.Clear();
            ...
            {
              reader.Read();
              blackElo = int.Parse(reader["Elo"].ToString());
              blackPlayer = int.Parse(reader["pID"].ToString());
            }
            command.Parameters.Clear();

            // FINAL QUERY SETUP
            command.CommandText = "Select eName, Site, Date, Result";
            if (showMoves == true)
              command.CommandText += ", Moves";
            command.CommandText += " from Games natural join Events where BlackPlayer = @blackPlayer and WhitePlayer = @whitePlayer";
            ...
            if (winner != "")
            {
                command.CommandText += (" and Result = @winner");
                command.Parameters.AddWithValue("@winner", winner[0]);
            }
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
              {
                while (reader.Read())
                {
                  ...
                  parsedResult += "White: " + white + " (" + whiteElo + ") \r\n";
                  parsedResult += "Black: " + black + " (" + blackElo + ") \r\n";  
